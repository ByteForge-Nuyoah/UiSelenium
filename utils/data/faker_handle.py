...
class FakerData:
    ...
    def generate_paragraph(self, lan: str = "en", nb: int = 3) -> str:
        ...
        """
        if lan == "zh":
            text = self.fk_zh.paragraph(
                nb_sentences=nb, variable_nb_sentences=True, ext_word_list=None
            )
        else:
    ...
    def generate_identifier(self, lan="en", char_len=8):
        ...
        while True:
            identifier = "".join(
                random.choices(string.ascii_letters + string.digits + "_.-", k=char_len)
            )  # 生成指定长度的随机标识

            if (
                re.match(r"^[a-zA-Z0-9][a-zA-Z0-9_.-]{0,98}[a-zA-Z0-9]$", identifier)
                and not (
