    ...
    sorted_files = sorted(
        [
            (
                ...
                os.path.getmtime(os.path.join(dir_path, file)),
            )
            for file in files
        ],
        key=lambda x: x[1],
...
def zip_file(in_path: str, out_path: str):
    ...
    if os.path.isdir(in_path):
        ...
        # 写入
        base_name = out_path.replace(".zip", "")
        shutil.make_archive(base_name, "zip", in_path)
...
def get_relative_path(file_path, directory_path):
    ...
    relative_path = os.path.relpath(
        os.path.abspath(file_path), os.path.abspath(directory_path)
    )
...
def file_to_base64(file_path):
    ...
    if os.path.exists(file_path):
        ...
        print(f"{file_path} 不是一个真实有效的文件路径")
