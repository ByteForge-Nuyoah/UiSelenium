...
class DingTalkBot:
    ...
    def send_message(self, payload):
        ...
        :payload: 请求json数据
        """
        logger.debug(f"准备发送钉钉消息: {payload.get('msgtype', 'unknown')}")
        response = request(
            url=self.webhook_url, json=payload, headers=self.headers, method="POST"
        )
    ...
    def send_action_card_single(
        self, title, text, single_title, single_url, btn_orientation=0
    ):
        """
    ...
    def send_action_card_split(self, title, text, btns, btn_orientation=0):
        ...
        for btn in btns:
            payload["actionCard"]["btns"].append(
                {"title": btn.get("title"), "actionURL": btn.get("action_url")}
            )
    ...
    def send_feed_card(self, links_msg):
        ...
        """
        payload = {"msgtype": "feedCard", "feedCard": {"links": []}}
