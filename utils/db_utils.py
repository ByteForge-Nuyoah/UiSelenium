...
class MysqlServer:
    ...
    def __init__(
        self, db_host, db_port, db_user, db_pwd, db_database, ssh=False, **kwargs
    ):
        """
        ...
        try:
            if ssh:
                ...
                self.server.start()
                db_host = (
                    self.server.local_bind_host
                )  # server.local_bind_host 是 参数 local_bind_address 的 ip
                db_port = (
                    self.server.local_bind_port
                )  # server.local_bind_port 是 参数 local_bind_address 的 port
            # 建立连接
    ...
    def __del__(self):
        ...
        try:
            ...
            logger.error("数据库连接关闭失败，原因 %s", error)
