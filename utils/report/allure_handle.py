...
from utils.file_utils import zip_file, copy_file
    ...
    if source:
        with allure.step(step_title):
            ...
            allure.attach.file(
                ...
                attachment_type=(
                    _attachment_type
                    if _attachment_type is None
                    else _attachment_type.value
                ),
...
class AllureReportBeautiful:
    ...
    def __init__(self, allure_html_path=None, allure_results_path=None):
        ...
        if os.path.exists(allure_html_path) and os.path.exists(allure_results_path):
            ...
            print(
                f"allure results以及allure html报告未生成~ \n"
    ...
    def set_report_env_on_results(self, env_info):
        ...
        with open(
            os.path.join(self.allure_results_path, "environment.properties"),
            "w",
            encoding="utf-8",
        ) as f:
    ...
    def set_report_env_on_html(self, env_info: dict):
        ...
        with open(
            os.path.join(self.allure_html_path, "widgets", "environment.json"),
            "w",
            encoding="utf-8",
        ) as f:
...
def generate_allure_report(**kwargs):
    ...
    attachment_path = kwargs.get(
        "attachment_path"
    )  # allure报告压缩的路径，例如：report/allure_report.zip
