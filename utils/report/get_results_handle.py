    ...
    try:
        ...
        if case_count["total"] > 0:
            ...
            test_results["pass_rate"] = round(
                (case_count["passed"] + case_count["skipped"])
                / case_count["total"]
                * 100,
                2,
            )
        ...
        test_results["run_time"] = (
            _time if test_results["total"] == 0 else round(_time["duration"] / 1000, 2)
        )
        test_results["start_time"] = timestamp_strftime(_time["start"])
        test_results["stop_time"] = timestamp_strftime(_time["stop"])

        # 收集重试次数
        retry_trend_json_path = os.path.join(
            allure_html_path, "widgets", "retry-trend.json"
        )
        ...
        logger.error(
            f"程序中检查到您未生成allure报告，通常可能导致的原因是allure环境未配置正确，{e}"
        )
